...
\begin{frame}{Lockdown and suspension of selection}
	...
	\begin{columns}
		...
		\begin{figure}
			\centering
			\scalebox{0.3}{\input{activity.pdf_tex}}
		\end{figure}

		\column{0.49\textwidth}
		\begin{figure}
			\centering
			\scalebox{0.3}{\input{rest.pdf_tex}}
		\end{figure}
	\end{columns}}
...
\end{figure}}
\footnotetext[1]{\tiny Image courtesy: Harshavardhan BV}
\end{frame}
...
\only<1-4>{\begin{block}{Trends across species}
	\begin{itemize}
		\item<1-> Body size as well as lifespan varies widely across species and taxa.
		\item<1-> Cancer risk is not correlated to this variation \citep{Peto2015}.
		\item<2-> Lack of data and multiple disparate lines of theoretical inquiry
		\item<3-> How does greater number of cells or cell division cycles not increase risk of mutation and cancer?
		\item<4-> Selection on somatic mutations and clonal expansion are uncommon in current theory \citep{Nowell1976}.
		\item<4-> Neutral vs non-neutral somatic mutation accumulation
	\end{itemize}
\end{block}}

\only<5->{\begin{block}{Within species-late-life decline}
	\begin{itemize}
		\item<5-> Incidence of cancer in humans rises near mid-life, peaks around 70 years and then declines \citep{Harding2012}.
		\item<6-> Reasons for late-life decline in incidence are unclear and links with late-life mortality plateaus are unexplored.
		\item<6-> What roles do temporal patterns of turnover play in mutation accumulation? \citep{Rozhok2019}
	\end{itemize}
\end{block}}

\only<7->{Work on developing an agent-based model based on \cite{Erten2020} for both these questions is ongoing with a Master's student.}
\end{frame}
...
\begin{frame}{Timeline}
	\begin{itemize}
		...
		\item Multiple lines of active work that would take at least a year to complete
	\end{itemize}
\end{frame}
